#!/usr/bin/env python3
"""
fetch_union.py v5.1.0 — Free API Live Football Data (RapidAPI)
 
BUGFIXES v5.1.0:
- home.id / away.id sind STRINGS in der API → alle Vergleiche mit str(UID)
- tournament.stage ist immer leer → Spieltagnummer aus Reihenfolge berechnet
- qualColor aus Standings für Tabellenfarben genutzt
 
Endpoints (~6 Requests/Tag):
...
BASE   = f"https://{HOST}"
LEAGUE = 54
UID    = 8149           # Integer für Standings
UID_S  = str(UID)       # String für Matches (home.id ist String!)
...
def result_char(m):
    """Ergebnis aus Sicht von Union (home.id ist STRING)."""
...
def main():
    ...
    # Tabellenumfeld ±2 (mit qualColor für Qualifikationsfärbung)
    context = []
    ...
    # home.id ist STRING → UID_S verwenden!
    ...
    # Spieltagnummer: Position in done-Liste (chronologisch)
    # done[0] = Spieltag 1, done[-1] = letzter Spieltag
    def matchday_of(m, in_done=True):
        lst = done if in_done else future
        try:
            idx = lst.index(m)
            base = len(done) if not in_done else 0
            return (base + idx + 1) if not in_done else (idx + 1)
        except:
            return "–"
 
    last_m = parse_match(done[-1], len(done))         if done   else None
    ...
    # Form aus letzten 5 Spielen berechnen (zuverlässiger als API-Feld)
    form_calc = ""
    for m in done[-5:]:
        c = result_char(m)
        if c: form_calc += c
    ...
    try:
        ...
        names=[p["name"]+" ("+str(p["goals"])+"G)" for p in top_scorers]
        print(f"   {names}")
    except Exception as e:
        print(f"   WARN: {e}", file=sys.stderr)
    ...
    try:
        ...
        names=[p["name"]+" ("+str(p["assists"])+"A)" for p in top_assisters]
        print(f"   {names}")
    except Exception as e:
        print(f"   WARN: {e}", file=sys.stderr)

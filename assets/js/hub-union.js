(() => {
  ...
  function headToHeadSection(opponent){
    ...
    const rows=opponent.headToHead.slice().reverse().map(match=>`<div class="uc-h2h-row"><span class="uc-opponent-result-state ${match.result.toLowerCase()}">${match.result==='W'?'S':match.result==='D'?'U':'N'}</span><div><b>${esc(match.isHome?'Heimspiel':'Auswärts')}</b><span>${new Intl.DateTimeFormat('de-DE',{day:'2-digit',month:'2-digit',year:'2-digit',timeZone:'Europe/Berlin'}).format(new Date(match.date))}</span></div><strong>${match.goalsFor}:${match.goalsAgainst}</strong></div>`).join('');
    return`<section class="uc-h2h"><div class="uc-h2h-title">Direkter Vergleich · letzte Duelle</div>${rows}<div class="uc-h2h-perspective">Ergebnisse aus Union-Sicht</div></section>`;
  }

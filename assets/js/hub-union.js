(() => {
  ...
  function opponentSection(opponent){
    if(!opponent||!Array.isArray(opponent.lastMatches)||!opponent.lastMatches.length)return'';
    const dots=[...opponent.form].map(result=>`<span class="uc-fd ${result==='W'?'w':result==='D'?'d':'l'}">${result==='W'?'S':result==='D'?'U':'N'}</span>`).join('');
    const wins=[...opponent.form].filter(result=>result==='W').length,draws=[...opponent.form].filter(result=>result==='D').length,losses=[...opponent.form].filter(result=>result==='L').length;
    const matches=opponent.lastMatches.slice().reverse().map(match=>`<article class="uc-opponent-result"><span class="uc-opponent-result-state ${match.result.toLowerCase()}">${match.result==='W'?'S':match.result==='D'?'U':'N'}</span>${logo(match.opponent)}<div><b>${esc(match.isHome?'vs. ':'bei ')}${esc(match.opponent.shortName||match.opponent.name)}</b><span>${new Intl.DateTimeFormat('de-DE',{day:'2-digit',month:'2-digit',year:'2-digit',timeZone:'Europe/Berlin'}).format(new Date(match.date))} · Bundesliga</span></div><strong>${match.goalsFor}:${match.goalsAgainst}</strong></article>`).join('');
    return`<section class="uc-opponent"><div class="uc-opponent-head"><div><div class="uc-opponent-kicker">Gegnercheck · nächstes Spiel</div><div class="uc-opponent-title">${logo(opponent)}<span>${esc(opponent.shortName||opponent.name)}</span></div></div><div class="uc-opponent-form"><div>${dots}</div><span>${wins}S · ${draws}U · ${losses}N</span></div></div><div class="uc-opponent-results"><div class="uc-box-lbl">Letzte 3 Bundesliga-Ergebnisse</div>${matches}</div></section>`;
  }

...
| `paul-gateway-v2 /finance` | FinanzenPaul | Lesen und Schreiben | vollständiger persönlicher Finanzplan | Google-Authentifizierung; streng validiertes und größenbegrenztes Schema; D1 ausschließlich als Worker-Binding; vorhandener JSONBin-Stand wird einmalig nach Anmeldung importiert | **Niedrig:** nach erfolgreichem Live-Test alten JSONBin-Schlüssel rotieren und lokale Altdaten entfernen. |
...
| JSONBin (temporäre Migrationsreserve) | einmaliger Altimport | Lesen | bestehender Master-Key und Bin-ID verbleiben bis zum bestätigten Live-Test lokal | **Mittel, befristet:** keine neuen Schreibzugriffe; nach bestätigter D1-Persistenz Schlüssel rotieren und lokale Werte entfernen. |

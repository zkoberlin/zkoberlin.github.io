...
Der Worker `paul-gateway-v2` ist seit dem 26.08.2026 der einzige öffentliche Einstiegspunkt für Hub und KalenderPaul. Das Frontend sendet einmalig ein Google-OAuth-Token; der Gateway prüft Identität und freigegebene E-Mail-Adresse und erstellt eine widerrufbare 30-Tage-Sitzung. Das zufällige Sitzungstoken liegt im Browser, in D1 ausschließlich dessen SHA-256-Hash. Mit einer gültigen Sitzung leitet der Gateway private Anfragen über das Cloudflare Service Binding `BACKEND` an `kalender-proxy` weiter; direkte Google-Tokens bleiben als kompatibler Übergang ebenfalls prüfbar. Die öffentliche `workers.dev`-Adresse von `kalender-proxy` ist deaktiviert (`workers_dev: false`), der Worker bleibt intern über das Service Binding erreichbar.

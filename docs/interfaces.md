...
Der Worker `paul-gateway-v2` ist seit dem 26.08.2026 der einzige öffentliche Einstiegspunkt für Hub und KalenderPaul. Das Frontend sendet einmalig ein Google-OAuth-Token; der Gateway prüft Identität und freigegebene E-Mail-Adresse und erstellt eine widerrufbare 30-Tage-Sitzung. Das zufällige Sitzungstoken liegt im Browser, in D1 ausschließlich dessen SHA-256-Hash. Mit einer gültigen Sitzung leitet der Gateway private Anfragen über das Cloudflare Service Binding `BACKEND` an `kalender-proxy` weiter; dabei entfernt er Browser-Zugangsdaten und authentifiziert sich intern mit einem gemeinsamen Secret. Direkte Google-Tokens bleiben am öffentlichen Gateway als kompatibler Übergang ebenfalls prüfbar. Die öffentliche `workers.dev`-Adresse von `kalender-proxy` ist deaktiviert (`workers_dev: false`), der Worker bleibt intern über das Service Binding erreichbar.
...
| `paul-gateway-v2 /auth/me` | Hub und KalenderPaul | Lesen | Kontoprofil zur Sitzungsprüfung | Prüfung der gehashten, nicht abgelaufenen D1-Sitzung | **Niedrig:** abgelaufene und widerrufene Sitzungen gezielt testen. |
| `paul-gateway-v2 /auth/session` | Hub, KalenderPaul und FinanzenPaul | Erstellen, Widerrufen | zufälliges Sitzungstoken, E-Mail-Adresse, Anzeigename und Ablaufzeit | Google-Prüfung bei Erstellung; 30 Tage Laufzeit; nur SHA-256-Hash in D1; Primär-Lesen gegen Replikatverzögerung; expliziter Logout | **Niedrig:** Browser-Speicher bleibt durch die statische, domainübergreifende Architektur nötig; CSP und Drittanbieter-Skripte weiter härten. |

...
function renderAll() {
  ...
  if(hdEl) hdEl.innerHTML = `${DN[(now.getDay()||7)-1]}, ${now.getDate()}. ${MN[now.getMonth()]} ${now.getFullYear()} <span class="version-badge version-badge-mobile" data-app-version>v${window.PAUL_APP_VERSION||'6.35.2'}</span>`;
  initViewUI();
  updateViewBtns();
  renderView();
}
